...
function diffSchema(other, options) {
    ...
    for (let key in this.getTypeMap()) {
        const synonym = getSynonym(options, {thisType: key}, 'otherType');
        const thisType = this.getTypeMap()[key];
        const otherType = other.getTypeMap()[synonym || key];
        diffs = diffs.concat(thisType.diff(otherType, options));
    }
    for (let key in other.getTypeMap()) {
        const synonym = getSynonym(options, {otherType: key}, 'thisType')
        const thisType = this.getTypeMap()[synonym || key];
        if (!thisType) {
...
function diffScalarTypes(other, options) {
    ...
    return descriptionDiff.call(this, other, DiffType.TypeDescriptionDiff, options);
}
...
function diffEnumTypes(other, options) {
    ...
    diffs = diffs.concat(descriptionDiff.call(this, other, DiffType.TypeDescriptionDiff, options));
    return dedupe(diffs);
}
...
function diffUnionTypes(other, options) {
    ...
    let diffs = descriptionDiff.call(this, other, DiffType.TypeDescriptionDiff, options);
...
function diffFields(thisType, otherType, options) {
    ...
    for (let key of Object.keys(thisType.getFields())) {
        const synonym = getSynonym(options, {thisType: thisType.name, thisField: key },'otherField');
        const thisField = thisType.getFields()[key];
        const otherField = otherType.getFields()[key] || otherType.getFields()[synonym];
        if (!otherField) {
        ...
        const fieldTypeSynonym = getSynonym(options, {thisType: thisField.type.name},'otherType');
        const thisTypeName = getFieldTypeName(thisField, fieldTypeSynonym);
        ...
        diffs = diffs.concat(descriptionDiff.call(thisField, otherField, DiffType.FieldDescriptionDiff, options, thisType.name+'.'+key));
    ...
    for (let key of Object.keys(otherType.getFields())) {
        const synonym = getSynonym(options, {thisType: thisType.name, otherField: key },'thisField');
        const otherField = otherType.getFields()[key];
        const thisField = thisType.getFields()[key] || thisType.getFields()[synonym];
        if (!thisField) {
...
function diffEnumValues(thisVals, otherVals, thisType, otherType, options) {
    ...
    thisVals.forEach(function(value, key) {
        if (!otherVals.get(key)) {
            ...
            diffs = diffs.concat(descriptionDiff.call(value, otherVals.get(key), DiffType.EnumDiff, options, thisType.name + '.' + thisVals.get(key).name));
 ...
 */
function getFieldTypeName(field, synonymOverride) {
    return buildTypeString(field.type, synonymOverride)
}

function buildTypeString(type, synonymOverride) {
    if (type instanceof GraphQLNonNull) {
        return buildTypeString(type.ofType, synonymOverride) + "!";
    } else if (type instanceof GraphQLList) {
        return "[" + buildTypeString(type.ofType, synonymOverride) + "]";
    } else {
        return synonymOverride || type.name;
    }
}
...
function descriptionDiff(other, type, options, fieldName) {
    let onClause = type === DiffType.FieldDescriptionDiff ? 'field'
        : type === DiffType.EnumDiff ? 'enum value'
        : 'type';
    if (this.description !== other.description) {
        const description = format('Description diff on {0} {1}. {2}: `"{3}"` vs. {4}: `"{5}"`.', onClause, fieldName || this.name, options.labelForThis, this.description, options.labelForOther, other.description);
        return [new GraphQLDiff(this, other, type, description, true)];
    }
    return [];
}

function commonTypeDiffs(other, options) {
    let synonym = getSynonym(options, {thisType: this.name}, 'otherType');
    if (!other) {
    ...
    if (this.name !== other.name && other.name !== synonym) {
...
function getSynonym(options, criteria, field) {
    let synonym =  find(options.synonyms, criteria);
    if(!synonym) return null;
    return synonym[field];
}
