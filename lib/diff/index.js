...
function diffFields(thisType, otherType, options) {
    ...
    for (let key of Object.keys(thisType.getFields())) {
        ...
        const otherField = otherType.getFields()[key] || otherType.getFields()[getSynonymFromThis(options, thisType.name, key)];
        if (!otherField) {
    ...
    for (let key of Object.keys(otherType.getFields())) {
        ...
        const thisField = thisType.getFields()[key] || thisType.getFields()[getSynonymFromOther(options, thisType.name, key)];
        if (!thisField) {
...
function getSynonymFromThis(options, type, field) {
    let synonym =  options.synonyms.find(x => x.type === type && x.this === field);
    if(!synonym) return null;
    return synonym.other;
}

function getSynonymFromOther(options, type, field) {
    let synonym =  options.synonyms.find(x => x.type === type && x.other === field);
    if(!synonym) return null;
    return synonym.this;
}
